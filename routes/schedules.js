...
router.post(
  "/:scheduleId/users/:userId/availabilities",
    ...
    Availability.findAll({
      ...
      if (availabilities) {
        availabilities.forEach((a) => {
          if (parseInt(a.userId) === parseInt(req.params.userId)) {
            a.update({
              scheduleId: req.params.scheduleId,
        ...
        res.json({ status: "OK", message: "updated all availabilities" });
      } else {
        const err = new Error("候補の更新に失敗しました");
        err.status = 404;
        next(err);
      }
    });
